...
- the current Vitest suite contains the runner-foundation smoke test plus the Phase 4 Domain, provider-neutral Application, row-decoder, and adapter-boundary tests described below; the latest measured run completed 29 files and 215 tests on 2026-08-31;
...
The current Domain and Application baseline was measured on 2026-08-31 after WP22 over the active `src/{application,domain}/**/*.ts` scope: Statements 96.84% (613/633), Branches 96.74% (625/646), Functions 100% (101/101), and Lines 98.53% (538/546). The suite completed 29 files and 215 tests. This is not a repository-wide quality claim; adapters and Edge Function code have separate mocked-boundary and local Supabase integration evidence and remain outside the numeric Domain/Application scope. The baseline does not cover the frozen legacy application, UI, hosted Supabase, or future Product integration. No numeric threshold or ratchet is configured yet; setting one requires separate approval.
