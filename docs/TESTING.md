...
- the current Vitest suite contains the runner-foundation smoke test plus the Phase 4 Domain and provider-neutral Inventory Application tests described under Domain Unit Tests below; the latest measured run completed 21 files and 123 tests on 2026-08-29;
...
The current Domain and Application baseline was measured on 2026-08-29 after the Phase 4 WP18 Inventory sort milestone over the active `src/{application,domain}/**/*.ts` scope: Statements 96.92% (535/552), Branches 96.81% (547/565), Functions 100% (87/87), and Lines 98.31% (468/476). The suite completed 21 files and 123 tests. This is not a repository-wide quality claim and does not cover the frozen legacy application, adapters, UI, live Supabase, or future Product integration. No numeric threshold or ratchet is configured yet; setting one requires separate approval.
