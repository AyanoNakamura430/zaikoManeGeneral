...
- the current Vitest suite contains the runner-foundation smoke test plus the Phase 4 Domain and provider-neutral Inventory Application tests described under Domain Unit Tests below; the latest measured run completed 21 files and 124 tests on 2026-08-30;
...
- the production-core local database harness applied the WP19 migration twice and completed 11 named reference-seed, GRANT/RLS, account-state, cross-account, Category/Item constraint, Unicode normalization, JSON, and timestamp assertions without changing protected workspace artifacts on 2026-08-30;
...
The current Domain and Application baseline was measured on 2026-08-30 after the Phase 4 WP19 persistence-boundary alignment over the active `src/{application,domain}/**/*.ts` scope: Statements 96.98% (547/564), Branches 96.86% (556/574), Functions 100% (87/87), and Lines 98.36% (480/488). The suite completed 21 files and 124 tests. This is not a repository-wide quality claim and does not cover the frozen legacy application, adapters, UI, live Supabase, or future Product integration. No numeric threshold or ratchet is configured yet; setting one requires separate approval.
