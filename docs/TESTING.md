...
The first production Domain slices are `normalizeCategoryNameKey`, the fixed-Unit inventory core, the Hybrid Attribute contract and codec, and active Attribute selection from a current nullable Template key. Named tests cover Category comparison normalization; the exact 18-Unit catalog and runtime immutability; Unit-key-bound opaque Quantity and threshold validation; finite, non-negative, count-integer, decimal-measurement, and negative-zero behavior; `out`, `low`, and `available` status derivation; the exact fixed Attribute Definition catalog; known-value type validation; valid unknown-JSON preservation; hidden Category namespace retention during non-destructive patching; immutable snapshot isolation; ordered current-Template definitions with optional values; hidden and unknown exclusion; null-only custom/Uncategorized handling; and unknown Template rejection. Search token extraction and Application/UI integration remain later slices. The separate `tests/unit/vitest-foundation.test.ts` smoke test remains excluded from production coverage and is not counted as Domain behavior coverage.
...
The current Domain baseline was measured on 2026-08-22 after the Phase 4 WP3 active-selection slice over the active `src/domain/**/*.ts` scope: Statements 100% (95/95), Branches 98.8% (83/84), Functions 100% (24/24), and Lines 100% (83/83). The suite completed 8 files and 33 tests. This is a Domain-only baseline, not a repository-wide quality claim, and it does not cover the frozen legacy application or future Product integration. No numeric threshold or ratchet is configured yet; setting one requires separate approval.
