...
- the current Vitest suite contains the runner-foundation smoke test plus the Phase 4 Domain, provider-neutral Application, row-decoder, and adapter-boundary tests described below; the latest measured run completed 31 files and 234 tests on 2026-08-31;
...
- the production-core local harness applied the migrations twice, completed local security/performance advisors with no error-level finding, and passed 18 named reference-seed, GRANT/RLS, account-state, cross-account, Category/Item constraint, authenticated adapter, Auth-eligibility, and trusted-onboarding assertions with Edge Runtime enabled and without changing protected workspace artifacts on 2026-08-31;
...
The current Domain and Application baseline was measured on 2026-08-31 after WP23 over the active `src/{application,domain}/**/*.ts` scope: Statements 96.96% (639/659), Branches 96.87% (650/671), Functions 100% (107/107), and Lines 98.59% (562/570). The suite completed 31 files and 234 tests. This is not a repository-wide quality claim; adapters and Edge Function code have separate mocked-boundary and local Supabase integration evidence and remain outside the numeric Domain/Application scope. The baseline does not cover the frozen legacy application, UI, hosted Supabase, or future Product integration. No numeric threshold or ratchet is configured yet; setting one requires separate approval.
