...
The first production Domain slices are `normalizeCategoryNameKey`, Category name validation and protection policy, the fixed-Unit inventory core, Item name and inventory-core validation, the Hybrid Attribute contract and codec, active Attribute selection from a current nullable Template key, pure Search query matching, Item Search-field assembly, the Auth return-target allowlist, the Inventory Category, stock-status, and Unit filter core, and the Purchase-date range filter core. Named tests cover Category comparison normalization; original display-name preservation; blank and normalized-duplicate rejection; opaque validated names; system/custom rename, reorder, delete, and color-change permissions; fail-closed unknown policy inputs; the exact 18-Unit catalog and runtime parsing; Unit-key-bound opaque Quantity and threshold validation; runtime-Unit-derived opaque Item cores; blank-only Item-name validation with original-name and duplicate-name preservation; no implicit defaults or stored status; finite, non-negative, count-integer, decimal-measurement, and negative-zero behavior; `out`, `low`, and `available` status derivation; the exact fixed Attribute Definition catalog; known-value type validation; valid unknown-JSON preservation; hidden Category namespace retention during non-destructive patching; immutable snapshot isolation; ordered current-Template definitions with optional values; hidden and unknown exclusion; null-only custom/Uncategorized handling; unknown Template rejection; NFKC/case/Unicode-whitespace Search normalization; first-occurrence token deduplication; empty-query behavior; multi-field token AND; field-boundary isolation; fixed common-field order; active searchable text Definition order; boolean/hidden/unknown/normalized-empty exclusion; custom Category-name inclusion; absence of a synthetic Uncategorized field; exact static and UUID-shaped return routes; opaque parse-only return targets; fail-closed rejection of public, external, traversal, encoded-separator, Unicode-separator, query, hash, control-character, malformed, and non-string return targets; immutable authentic filter instances; Category/stock-status/Unit OR semantics; cross-dimension AND semantics; Uncategorized selection; derived stock-status matching; fail-closed malformed filter and Item contracts; strict civil Gregorian `YYYY-MM-DD` dates in years 0001–9999; inclusive optional date bounds; reversed-range rejection; and authentic immutable date ranges with fail-closed malformed inputs. Application/UI integration remains a later slice. Filter composition with Search remains a later slice. The separate `tests/unit/vitest-foundation.test.ts` smoke test remains excluded from production coverage and is not counted as Domain behavior coverage.
...
The current Domain baseline was measured on 2026-08-23 after the Phase 4 WP10 Purchase-date filter slice over the active `src/domain/**/*.ts` scope: Statements 99.31% (290/292), Branches 98.95% (283/286), Functions 100% (56/56), and Lines 100% (245/245). The suite completed 15 files and 86 tests. This is a Domain-only baseline, not a repository-wide quality claim, and it does not cover the frozen legacy application or future Product integration. No numeric threshold or ratchet is configured yet; setting one requires separate approval.
