...
The Phase 2 quality architecture is approved. The artifact-safe verification shell, strict TypeScript, lint/format, unit, Component/Router integration, ephemeral Supabase security, generated-type drift, Playwright E2E, and GitHub Actions CI foundations through Phase 3 WP10 are implemented. Phase 4 now includes Domain and provider-neutral Inventory/Auth Application tests, the production Inventory-core migration, Supabase read and eligibility adapters, the WP22 trusted-onboarding Edge Function with local integration evidence, WP23 provider-neutral Auth-bootstrap orchestration tests, the WP24 Auth composition boundary with local end-to-end adapter evidence, and the WP25 browser Supabase configuration/client factory. The GitHub-hosted workflow has not yet been observed running; Product UI/Router, hosted-service integration, remaining write adapters, and deployment validation remain future work.
...
- the current Vitest suite contains the runner-foundation smoke test plus the Phase 4 Domain, provider-neutral Application, composition, row-decoder, and adapter-boundary tests described below; the latest measured run completed 34 files and 253 tests on 2026-08-31;
...
- the production-core local harness applied the migrations twice, completed local security/performance advisors with no error-level finding, and passed 19 named reference-seed, GRANT/RLS, account-state, cross-account, Category/Item constraint, authenticated adapter, Auth-eligibility, trusted-onboarding, and Auth-composition assertions with Edge Runtime enabled and without changing protected workspace artifacts on 2026-08-31;
...
WP8 originally applied the WP7 test-only migration in a unique OS-temporary local Supabase project with pinned CLI `supabase@2.115.0`. WP19 keeps that historical fixture but moves the canonical `verify:database-types` source to the repository-local production migration and compares it with `src/infrastructure/supabase/database.generated.ts` without writing the repository. `verify:database-types-self-test` accepts an exact temporary candidate and rejects an intentionally drifted candidate. Only `update:database-types` writes the production baseline, after successful generation and local-stack cleanup.

The original WP8 file under `tests/supabase/generated` remains the test-harness contract. WP19 redirects the canonical generation and drift check to the local production migration and commits its output under `src/infrastructure/supabase/database.generated.ts`. The generated header prohibits manual edits and carries only the narrow lint exception required by the generator output. Neither generated file establishes live-schema parity or authorizes remote/linked type generation.
...
`src/lib/supabase.ts` is a temporary direct-Supabase exception for the frozen legacy application. Remove the exception when the new composition root replaces the legacy client entry; new Product code must not import it. `src/vite-env.d.ts` declares only the public Vite environment key names and their types; it contains no values. WP25 adds the optional publishable-key name while preserving the required legacy anon name for the still-frozen legacy client. Its injected configuration decoder permits HTTPS and loopback HTTP, rejects credential-bearing or non-browser URLs, prefers a non-empty `sb_publishable_` key, accepts a structurally identified legacy `anon` JWT only as fallback, rejects identifiable `sb_secret_` or legacy `service_role` input, and returns value-free errors. It decodes the legacy role only as a local misconfiguration guard and does not verify the JWT signature. These guards prevent common configuration mistakes but do not replace RLS or prove the privilege of an arbitrary undisclosed credential. `npm run verify:lint-gates` checks five negative and three positive virtual source cases through ESLint's `lintText` API using an isolated architecture-only config, without creating workspace fixtures or weakening type-aware lint for real source.
...
The first production slices include `normalizeCategoryNameKey`, Category name validation and protection policy, the fixed-Unit inventory core, Item name and inventory-core validation, the Hybrid Attribute contract and codec, active Attribute selection, pure Search matching and field assembly, the Auth return-target allowlist, Inventory Category/stock-status/Unit and Purchase-date filters, the composed Inventory query, strict canonical Instants, deterministic Inventory sorting, provider-neutral Application errors and collection states, and the Inventory read port and list use case. WP20 adds an immutable Inventory list read model plus a fail-closed database-row decoder that validates owner identity, Item/Category relation integrity, Unit and exact numeric contracts, Hybrid JSON, purchase dates, and Postgres RFC 3339 timestamps before canonicalizing Instants. Named Application tests distinguish authentication expiry, load errors, true empty, no results, and loaded Items; use a provider-neutral read port with no caller-supplied owner ID; apply Search and Filter before the requested deterministic sort; call the port once; reject incompatible Quantity sorts before calling the port; prevent raw provider details from escaping; and convert malformed contracts into integrity failure. WP23 adds canonical immutable Auth-bootstrap outcomes and verifies that non-onboarding eligibility skips onboarding, missing and pending accounts invoke it exactly once, successful onboarding reloads authoritative eligibility, only a refreshed active state resolves, malformed results fail closed, and initial eligibility, onboarding, and post-onboarding failures remain distinguishable. WP24 verifies that one injected typed browser client is used for eligibility, trusted onboarding, and refreshed eligibility, that active eligibility skips the Function, and that a real local missing-account flow converges through the composition boundary to an active account with six system Categories. The Supabase adapter server-verifies the current user, uses an explicit relational select under RLS, verifies returned ownership again while decoding, and maps Auth, transport, provider, and integrity failures without exposing raw responses. UI, Router, live Supabase, write adapters, and application-shell composition remain later slices. The separate `tests/unit/vitest-foundation.test.ts` smoke test remains excluded from production coverage and is not counted as Product behavior coverage.
...
WP19 adds the separate `verify:database-production` gate. It copies the production `supabase/` project into a unique validated OS-temporary directory, disables CLI telemetry, replaces the local project identity, resets the production migrations twice, runs local security/performance advisors with error-level failure, executes the dedicated black-box matrix, and removes the local stack in `finally`. WP20 extends it with the Inventory adapter and WP21 with Auth eligibility. WP22 enables the copied project's Edge Runtime and verifies anonymous rejection, deleting-account closure, missing creation, pending resumption including a three-Category checkpoint, active idempotency, repeated and concurrent invocation, exact Category key/name/order materialization, owner separation, and the browser adapter/Application path. WP24 adds the injected-client composition path from a missing account through the Function and refreshed eligibility to active, including the resulting six Categories. Supabase does not issue a password session to the deliberately unverified local fixture, so the defensive verification branch remains mocked-boundary evidence rather than a fabricated local session. The harness neither links to nor changes a remote project and does not cover Storage or hosted Function deployment.

The WP22 Function is formatted by the canonical format gate and compiled/executed by the pinned local Supabase Edge Runtime during `verify:database-production`. A standalone Deno executable is not installed in the verified Windows environment, so there is no separate repository-level Deno type/lint command; hosted-runtime parity and future Deno/API drift remain deployment-stage risks.
...
The current Domain and Application baseline was measured on 2026-08-31 after WP25 over the active `src/{application,domain}/**/*.ts` scope: Statements 96.96% (639/659), Branches 96.87% (650/671), Functions 100% (107/107), and Lines 98.59% (562/570). The suite completed 34 files and 253 tests. This is not a repository-wide quality claim; composition, adapters, and Edge Function code have separate unit or local Supabase integration evidence and remain outside the numeric Domain/Application scope. The baseline does not cover the frozen legacy application, UI, hosted Supabase, or future Product integration. No numeric threshold or ratchet is configured yet; setting one requires separate approval.
...
- `npm run verify:database-production` (`npm.cmd run verify:database-production` from Windows PowerShell) runs the local-only WP19 production migration and Inventory-core security matrix with the same cleanup boundary.
- `npm run verify:database-types` (`npm.cmd run verify:database-types` from Windows PowerShell) regenerates production types from local migrations into validated OS-temporary storage and performs a byte-for-byte drift check.
