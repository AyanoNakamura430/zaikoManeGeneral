...
The first production Domain slices are `normalizeCategoryNameKey`, the fixed-Unit inventory core, the Hybrid Attribute contract and codec, active Attribute selection from a current nullable Template key, and pure Search query matching. Named tests cover Category comparison normalization; the exact 18-Unit catalog and runtime immutability; Unit-key-bound opaque Quantity and threshold validation; finite, non-negative, count-integer, decimal-measurement, and negative-zero behavior; `out`, `low`, and `available` status derivation; the exact fixed Attribute Definition catalog; known-value type validation; valid unknown-JSON preservation; hidden Category namespace retention during non-destructive patching; immutable snapshot isolation; ordered current-Template definitions with optional values; hidden and unknown exclusion; null-only custom/Uncategorized handling; unknown Template rejection; NFKC/case/Unicode-whitespace Search normalization; first-occurrence token deduplication; empty-query behavior; multi-field token AND; and field-boundary isolation. Item Search-field assembly and Application/UI integration remain later slices. The separate `tests/unit/vitest-foundation.test.ts` smoke test remains excluded from production coverage and is not counted as Domain behavior coverage.
...
The current Domain baseline was measured on 2026-08-22 after the Phase 4 WP4 Search slice over the active `src/domain/**/*.ts` scope: Statements 100% (107/107), Branches 98.86% (87/88), Functions 100% (29/29), and Lines 100% (94/94). The suite completed 9 files and 39 tests. This is a Domain-only baseline, not a repository-wide quality claim, and it does not cover the frozen legacy application or future Product integration. No numeric threshold or ratchet is configured yet; setting one requires separate approval.
