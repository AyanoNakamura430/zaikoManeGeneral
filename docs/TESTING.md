...
The first production Domain slices are `normalizeCategoryNameKey`, Category name validation and protection policy, the fixed-Unit inventory core, Item name and inventory-core validation, the Hybrid Attribute contract and codec, active Attribute selection from a current nullable Template key, pure Search query matching, and Item Search-field assembly. Named tests cover Category comparison normalization; original display-name preservation; blank and normalized-duplicate rejection; opaque validated names; system/custom rename, reorder, delete, and color-change permissions; fail-closed unknown policy inputs; the exact 18-Unit catalog and runtime parsing; Unit-key-bound opaque Quantity and threshold validation; runtime-Unit-derived opaque Item cores; blank-only Item-name validation with original-name and duplicate-name preservation; no implicit defaults or stored status; finite, non-negative, count-integer, decimal-measurement, and negative-zero behavior; `out`, `low`, and `available` status derivation; the exact fixed Attribute Definition catalog; known-value type validation; valid unknown-JSON preservation; hidden Category namespace retention during non-destructive patching; immutable snapshot isolation; ordered current-Template definitions with optional values; hidden and unknown exclusion; null-only custom/Uncategorized handling; unknown Template rejection; NFKC/case/Unicode-whitespace Search normalization; first-occurrence token deduplication; empty-query behavior; multi-field token AND; field-boundary isolation; fixed common-field order; active searchable text Definition order; boolean/hidden/unknown/normalized-empty exclusion; custom Category-name inclusion; and absence of a synthetic Uncategorized field. Application/UI integration remains a later slice. The separate `tests/unit/vitest-foundation.test.ts` smoke test remains excluded from production coverage and is not counted as Domain behavior coverage.
...
The current Domain baseline was measured on 2026-08-22 after the Phase 4 WP7 Item-core slice over the active `src/domain/**/*.ts` scope: Statements 100% (156/156), Branches 99.2% (125/126), Functions 100% (37/37), and Lines 100% (134/134). The suite completed 12 files and 62 tests. This is a Domain-only baseline, not a repository-wide quality claim, and it does not cover the frozen legacy application or future Product integration. No numeric threshold or ratchet is configured yet; setting one requires separate approval.
