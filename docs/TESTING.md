...
- the current Vitest suite contains the runner-foundation smoke test plus the Phase 4 Domain, provider-neutral Application, composition, row-decoder, and adapter-boundary tests described below; the latest measured run completed 32 files and 236 tests on 2026-08-31;
...
The current Domain and Application baseline was measured on 2026-08-31 after WP24 over the active `src/{application,domain}/**/*.ts` scope: Statements 96.96% (639/659), Branches 96.87% (650/671), Functions 100% (107/107), and Lines 98.59% (562/570). The suite completed 32 files and 236 tests. This is not a repository-wide quality claim; composition, adapters, and Edge Function code have separate unit or local Supabase integration evidence and remain outside the numeric Domain/Application scope. The baseline does not cover the frozen legacy application, UI, hosted Supabase, or future Product integration. No numeric threshold or ratchet is configured yet; setting one requires separate approval.
