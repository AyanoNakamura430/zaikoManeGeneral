...
- the current Vitest suite contains the runner-foundation smoke test plus the Phase 4 Domain and provider-neutral Inventory Application tests described under Domain Unit Tests below; the latest measured run completed 18 files and 106 tests on 2026-08-23;
...
The current Domain and Application baseline was measured on 2026-08-23 after the Phase 4 WP14 provider-neutral Inventory read milestone over the active `src/{application,domain}/**/*.ts` scope: Statements 97.16% (411/423), Branches 97.34% (404/415), Functions 100% (77/77), and Lines 98.6% (354/359). The suite completed 18 files and 106 tests. This is not a repository-wide quality claim and does not cover the frozen legacy application, adapters, UI, live Supabase, or future Product integration. No numeric threshold or ratchet is configured yet; setting one requires separate approval.
