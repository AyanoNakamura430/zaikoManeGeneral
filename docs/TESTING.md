...
The Phase 2 quality architecture is approved. The artifact-safe verification shell, strict TypeScript, lint/format, unit, Component/Router integration, ephemeral Supabase security, generated-type drift, Playwright E2E, and GitHub Actions CI foundations through Phase 3 WP10 are implemented. Phase 4 now includes Domain and provider-neutral Inventory and Auth-eligibility Application unit tests, the local production Inventory-core database milestone, the WP20 Supabase Inventory read adapter, and the WP21 Auth/Application Account eligibility bootstrap with local integration evidence. The GitHub-hosted workflow has not yet been observed running; Product UI/Router, live-service integration, write adapters, trusted onboarding, and deployment validation remain future work.
...
- the current Vitest suite contains the runner-foundation smoke test plus the Phase 4 Domain, provider-neutral Application, row-decoder, and adapter-boundary tests described below; the latest measured run completed 26 files and 194 tests on 2026-08-31;
...
- the production-core local database harness applied the WP19 and WP20 migrations twice, completed local security/performance advisors with no error-level finding, and passed 13 named reference-seed, GRANT/RLS, account-state, cross-account, Category/Item constraint, JavaScript-safe numeric boundary, Unicode normalization, JSON, timestamp, authenticated relational-read, and Auth-eligibility assertions without changing protected workspace artifacts on 2026-08-31;
...
WP19 adds the separate `verify:database-production` gate. It copies the production `supabase/` project into a unique validated OS-temporary directory, disables CLI telemetry, replaces the local project identity, resets the production migrations twice, runs local security/performance advisors with error-level failure, executes the dedicated black-box matrix, and removes the local stack in `finally`. The matrix covers exact reference seeds, anonymous denial, active/pending/deleting account behavior, owner and cross-account Category/Item behavior, system-Category protection, fixed Units and exact numeric boundaries, Hybrid JSON known-type enforcement and unknown preservation, server-owned timestamps, and byte parity between database and Domain Category normalization. WP20 extends the same local matrix with the less-than-one-billion six-decimal JavaScript-safe amount boundary, server user verification, the explicit Item/Category relational read, runtime mapping, anonymous rejection, and User A/B result isolation. WP21 adds anonymous, missing-account, pending-account, active User A/B, and deleting-account Auth-eligibility assertions. Supabase does not issue a password session to the deliberately unverified local fixture, so the defensive `verification_required` adapter branch is verified at the mocked provider boundary rather than by fabricating a local session. The harness neither links to nor changes a remote project and does not cover Storage or trusted onboarding execution.
...
The current Domain and Application baseline was measured on 2026-08-31 after WP21 over the active `src/{application,domain}/**/*.ts` scope: Statements 96.83% (582/601), Branches 96.73% (592/612), Functions 100% (96/96), and Lines 98.46% (512/520). The suite completed 26 files and 194 tests. This is not a repository-wide quality claim; Supabase adapters and row decoders are correctly located in the Adapter layer and have separate mocked-boundary and local Supabase integration evidence, so they remain outside the numeric Domain/Application coverage scope. The baseline does not cover the frozen legacy application, UI, live Supabase, or future Product integration. No numeric threshold or ratchet is configured yet; setting one requires separate approval.
