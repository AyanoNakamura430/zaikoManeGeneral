...
- the current Vitest suite contains the runner-foundation smoke test plus the Phase 4 Domain, provider-neutral Inventory Application, row-decoder, and adapter-boundary tests described below; the latest measured run completed 23 files and 154 tests on 2026-08-30;
...
- the production-core local database harness applied the WP19 and WP20 migrations twice, completed local security/performance advisors with no error-level finding, and passed 12 named reference-seed, GRANT/RLS, account-state, cross-account, Category/Item constraint, JavaScript-safe numeric boundary, Unicode normalization, JSON, timestamp, and authenticated relational-read adapter assertions without changing protected workspace artifacts on 2026-08-30;
...
The current Domain and Application baseline was measured on 2026-08-31 after the Phase 4 WP20 review corrections over the active `src/{application,domain}/**/*.ts` scope: Statements 96.98% (547/564), Branches 96.86% (556/574), Functions 100% (87/87), and Lines 98.36% (480/488). The suite completed 23 files and 154 tests. This is not a repository-wide quality claim; the Supabase row decoder is correctly located in the Adapter layer and has separate mocked-boundary and local Supabase integration evidence, so it remains outside the numeric Domain/Application coverage scope. The baseline does not cover the frozen legacy application, UI, live Supabase, or future Product integration. No numeric threshold or ratchet is configured yet; setting one requires separate approval.
