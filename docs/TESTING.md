...
The current Domain baseline was measured on 2026-08-23 after the Phase 4 WP9 Inventory-filter slice over the active `src/domain/**/*.ts` scope: Statements 99.17% (239/241), Branches 98.53% (202/205), Functions 100% (50/50), and Lines 100% (203/203). The suite completed 14 files and 78 tests. This is a Domain-only baseline, not a repository-wide quality claim, and it does not cover the frozen legacy application or future Product integration. No numeric threshold or ratchet is configured yet; setting one requires separate approval.
