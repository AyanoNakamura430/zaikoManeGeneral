...
The target frontend architecture is approved and partially implemented through pure Domain modules, provider-neutral Application errors and collection states, Inventory, Auth-eligibility, trusted-onboarding, and Auth-bootstrap use cases, their Supabase adapters, and the Auth bootstrap composition boundary. The Inventory adapter server-verifies the current user and relies on owner-scoped RLS. The Auth adapter treats `getSession()` only as stored-session presence, uses `getUser()` for server verification, and distinguishes missing from pending onboarding. The onboarding adapter invokes the authenticated trusted Function and accepts only its exact active result; raw Function errors do not reach UI. The Auth-bootstrap orchestrator runs onboarding exactly once only when eligibility requires it, reloads authoritative eligibility, requires the refreshed state to be active, and distinguishes initial eligibility, onboarding, and post-onboarding failures. The composition boundary injects one typed browser client into both adapters and exposes only the provider-neutral bootstrap outcome. No new application shell, Router integration, or Product screen invokes this boundary yet. The current `src/app/App.tsx` still couples UI, state, validation, Auth, CRUD, Storage, Search, Filter, Sort, and LocalStorage fallback. See [`CURRENT_STATE.md`](CURRENT_STATE.md).
