...
The target frontend architecture is approved and partially implemented through pure Domain modules, provider-neutral Application errors and collection states, Inventory, Auth-eligibility, and trusted-onboarding ports/use cases, and their Supabase adapters. The Inventory adapter server-verifies the current user and relies on owner-scoped RLS. The Auth adapter treats `getSession()` only as stored-session presence, uses `getUser()` for server verification, and distinguishes missing from pending onboarding. The onboarding adapter invokes the authenticated trusted Function and accepts only its exact active result; raw Function errors do not reach UI. No composition root, new application shell, Router integration, or Product screen uses these boundaries yet. The current `src/app/App.tsx` still couples UI, state, validation, Auth, CRUD, Storage, Search, Filter, Sort, and LocalStorage fallback. See [`CURRENT_STATE.md`](CURRENT_STATE.md).
