...
The target frontend architecture is approved and partially implemented through pure Domain modules, provider-neutral Application errors and collection states, an Inventory read port with no caller-supplied owner ID, the Inventory list use case, and a Supabase Inventory read adapter. The adapter server-verifies the current user, relies on owner-scoped RLS rather than a caller-supplied owner ID, performs one explicit relational Item/Category read, runtime-decodes rows into an independent immutable read model, and translates provider failures into the approved Application error codes. No composition root, new application shell, Router integration, or Product screen uses the adapter yet. The current `src/app/App.tsx` still couples UI, state, validation, Auth, CRUD, Storage, Search, Filter, Sort, and LocalStorage fallback. See [`CURRENT_STATE.md`](CURRENT_STATE.md).
