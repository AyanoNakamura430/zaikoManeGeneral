...
The target frontend architecture is approved and partially implemented through pure Domain modules, provider-neutral Application errors and collection states, Inventory and Auth-eligibility ports/use cases, and their Supabase adapters. The Inventory adapter server-verifies the current user, relies on owner-scoped RLS rather than a caller-supplied owner ID, performs one explicit relational Item/Category read, runtime-decodes rows into an independent immutable read model, and translates provider failures into approved Application error codes. The Auth adapter treats `getSession()` only as a stored-session presence check, uses `getUser()` for server verification, then derives the Application Account eligibility state from an owner-scoped read. Missing and pending accounts remain distinct onboarding reasons; the adapter does not execute trusted onboarding. No composition root, new application shell, Router integration, or Product screen uses either adapter yet. The current `src/app/App.tsx` still couples UI, state, validation, Auth, CRUD, Storage, Search, Filter, Sort, and LocalStorage fallback. See [`CURRENT_STATE.md`](CURRENT_STATE.md).
