...
WP19 persists Quantity and threshold as unconstrained exact `numeric` values with non-negative, at-most-14-integer-digit, and maximum-six-decimal-place checks; count Units additionally require integers. The Domain applies the same accepted-number boundary before persistence. Category `name_key` is generated by the database and is black-box checked against the Domain NFKC, Unicode whitespace, trim, and lowercase corpus. `color_key` and `image_path` remain NULL-only until their separately approved palette and Storage milestones.
